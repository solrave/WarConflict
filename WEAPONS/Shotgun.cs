...
public class Shotgun : Weapon
{
    ...
    public override void Shoot(IHittable target, Team team)
    {
        ...
        foreach (var indirectTarget in targetList) indirectTarget.TakeHit(Helper.GetRandomValue(SplashDamage));
    }

    private IEnumerable<IHittable> PickTargetsToSplash(IHittable target, Team team)
    {
        var index = team.Squad.Select((unit, index) => new { unit, index })
